...
## Contents of `.agent/skills/math-tutor/skill.md`
...
Auto-trigger test input:

```text
math tutor: explain why coercivity matters in Lax-Milgram.
```

Expected result: the Math Tutor skill suggestion appears because the input uses
the trigger keyword `math tutor`.

Slash-command test input:

```text
/math-tutor Explain the Lax-Milgram theorem with one PDE example.
```

Expected result: the Math Tutor skill is invoked directly through the
`/math-tutor` slash command.
